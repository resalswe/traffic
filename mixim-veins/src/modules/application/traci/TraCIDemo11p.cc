...
void TraCIDemo11p::handleParkingUpdate(cObject* obj) {
	...
	if (sendWhileParking == false) {
		if (isParking == true) {
			...
			(FindModule<BaseConnectionManager*>::findGlobalModule())->registerNic(this->getParentModule()->getSubmodule("nic"), (ConnectionManagerAccess*) this->getParentModule()->getSubmodule("nic")->getSubmodule("phy80211p"), &pos);
		}
	}
}
