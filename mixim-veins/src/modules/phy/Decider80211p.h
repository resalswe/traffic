...
class Decider80211p: public BaseDecider {
	...
	protected:
		...
		MiximAirFrame *curSyncFrame;

		std::string myPath;
		Decider80211pToPhy80211pInterface* phy11p;
		std::map<MiximAirFrame*,int> signalStates;
	...
	protected:
		 ...
		 */
		virtual BaseDecider::channel_sense_rssi_t calcChannelSenseRSSI(simtime_t_cref min, simtime_t_cref max) const;
		 ...
		 *
		 */
		virtual void calculateSinrAndSnrMapping(MiximAirFrame* frame, Mapping **sinrMap, Mapping **snrMap);
		 ...
		 */
		Mapping* calculateNoiseRSSIMapping(simtime_t_cref start, simtime_t_cref end, MiximAirFrame *frame);
	...
	public:
		...
		bool cca(simtime_t_cref, MiximAirFrame*);
		BaseDecider::eSignalState getSignalState(MiximAirFrame* frame) const;
