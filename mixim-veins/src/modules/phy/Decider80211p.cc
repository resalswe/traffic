...
simtime_t Decider80211p::processNewSignal(MiximAirFrame* msg) {
...
//			setChannelIdleStatus(false); FIXME: Is this correct
		}
...
//		setChannelIdleStatus(false); FIXME: Is this correct
		...
		if (phy11p->getRadioState() == MiximRadio::TX) {
...
BaseDecider::eSignalState Decider80211p::getSignalState(MiximAirFrame* frame) const {

	if (signalStates.find(frame) == signalStates.end()) {
		return NEW;
	}
	else {
		return (eSignalState)signalStates.find(frame)->second;
	}
}

BaseDecider::channel_sense_rssi_t Decider80211p::calcChannelSenseRSSI(simtime_t_cref start, simtime_t_cref end) const {

	BaseDecider::rssi_mapping_t rssiMapping = calculateRSSIMapping(start, end);
	...
	Mapping::argument_value_t rssi = MappingUtils::findMax(*(rssiMapping.first), min, max);

	delete rssiMapping.first;

	return std::make_pair(rssi, rssiMapping.second);
}

void Decider80211p::calculateSinrAndSnrMapping(MiximAirFrame* frame, Mapping **sinrMap, Mapping **snrMap) {
	...
	BaseDecider::rssi_mapping_t noiseInterferenceMap = calculateRSSIMapping(start, end, frame);
	assert(noiseInterferenceMap.first);
	...
	const ConstMapping* recvPowerMap = signal.getReceivingPower();
	assert(recvPowerMap);

	//TODO: handle noise of zero (must not divide with zero!)
	*sinrMap = MappingUtils::divide( *recvPowerMap, *(noiseInterferenceMap.first), Argument::MappedZero );
	*snrMap = MappingUtils::divide( *recvPowerMap, *noiseMap, Argument::MappedZero );

	delete noiseInterferenceMap.first;
	noiseInterferenceMap.first = NULL;
	delete noiseMap;
	noiseMap = NULL;

}

Mapping* Decider80211p::calculateNoiseRSSIMapping(simtime_t_cref start, simtime_t_cref end, MiximAirFrame *exclude) {
...
DeciderResult* Decider80211p::checkIfSignalOk(MiximAirFrame* frame) {
...
enum Decider80211p::PACKET_OK_RESULT Decider80211p::packetOk(double snirMin, double snrMin, int lengthMPDU, double bitrate) {
	...
	double packetOkSinr = 0;
	double packetOkSnr = 0;
...
bool Decider80211p::cca(simtime_t_cref time, MiximAirFrame* exclude) {
...
simtime_t Decider80211p::processSignalEnd(MiximAirFrame* msg) {
	...
	else if (frame->getWasTransmitting() || phy11p->getRadioState() == MiximRadio::TX) {
...
// FIXME: Shall this be used? 511, 502, 53, 59 see other TODO:s
